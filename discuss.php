...
<div class="jumbotron" style="margin-top:-19px;">
              <center>  <h2><b>Welcome to Discussion...</b></h2><br>
                <p class="lead">This is a Unique Platform for discussion of various counselling related queries. One can discuss ideas, ask queries and help each other.<br><strong>GIVE HELP.&emsp;  GET HELP.<br> and build a brand for yourself by contributing.</strong></p>
			   </center>
            </div>
     </div>
...
$sql="select question.*,users.name,users.email
from question,users
...
if($result==FALSE)
{
    ...
    {   
        ...
        echo "<table class='table table-striped' style='border-style:outset;'> 
	<tr><th>Question</th>
    ...
    while ($row=mysqli_fetch_array($result))
    {
        ...
        $em=$row['email'];
	echo "<tr>";
	echo "<td><strong><h4><a style='color:inherit;font-family:Times New Roman;' href = 'answerpage.php?var=$key1'>" .ucwords( $row['question'] ). "</a></h4></strong></td>";

	echo "<td><a href = 'who.php?var=$em'>" . ucwords($row['name']) . "</a></td>";
	echo "</tr>";  
} 
echo "</table></center>"; 
    }
